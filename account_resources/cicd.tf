####
# Hard coded to the DEV VPCs as that kicks off the CI/CD.
# And for MAIN releases it is hardcoded directly to the PROD VPCID / SubnetIDs
# means if we want to use this we should disable the entire module till the Env resources run once to setup DEV.
# Then need to update this script with the new environments vpc_id / subnet_ids

# ========================================
# CICD
# ========================================
#locals {
#  tango_infra_branch_develop = "develop"
#  tango_infra_branch_main    = "main"
#}
#
#module "cicd_develop" {
#  source = "../modules/cicd"
...
#    "github_branch"       = local.tango_infra_branch_develop
#    "github_repo"         = "infra"
...
//    "github_branch"       = local.tango_infra_branch_main
//    "github_repo"         = "infra"
//    "github_access_token" = data.external.github_access_token.result["token"]
//    "github_certificate"  = "${aws_s3_bucket.cicd_bucket.arn}/${aws_s3_bucket_object.github_cert.id}"
//
